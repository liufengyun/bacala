...
class MavenFetcherSuite extends BasicSuite {
  ...
  test("generate correct package POM URL") {
    ...
    val p = JPackage(JLib("org.scala-lang", "scala-library"), "2.11.5")
  ...
  test("generate correct library meta-data XML URL") {
    ...
    assert(fetcher.metaDataURL(JLib("org.scala-lang", "scala-library")) ===
  ...
  test("fetch meta data XML") {
    val file = Workers.DefaultMetaFetcher(JLib("org.scala-lang", "scala-library"))
    assert(file.nonEmpty)
    assert((XML.loadString(file.get) \ "versioning" \ "versions").length > 0)
  }

  test("fetch in-existent file") {
    val file = Workers.DefaultMetaFetcher(JLib("org.scala-lang", "foo-library"))
    assert(file.isEmpty)
  }
